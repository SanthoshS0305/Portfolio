...
export const renderSocialIcon = (iconKey) => {
  const icon = SOCIAL_ICONS[iconKey];
  if (!icon) return SOCIAL_ICONS.globe.svg;
  return icon.svg;
};
      ...
      {Object.entries(SOCIAL_ICONS).map(([key, { label, svg }]) => (
        <button
          ...
          style={{
            ...
            border: value === key
              ? '2px solid #FFD873'
              : '2px solid rgba(247,247,247,0.15)',
            borderRadius: '8px',
            background: value === key
              ? 'rgba(255,216,115,0.12)'
              : 'rgba(255,255,255,0.05)',
            color: value === key ? '#FFD873' : '#F7F7F7',
            cursor: 'pointer',
            fontSize: '11px',
            minWidth: '64px',
