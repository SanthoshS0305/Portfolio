...
const EMPTY = { title: '', alt: '', description: '', src: '', order: 0 };
...
const CarouselEditor = ({ onFeedback }) => {
  ...
  const saveItem = async () => {
    ...
    try {
      if (isNew) {
        await writeClient.create({ _type: 'carouselItem', ...editingDoc() });
      } else {
        await writeClient.patch(editing._id).set(editingDoc()).commit();
      }
      await load();
      setEditing(null);
      onFeedback('Carousel item saved!', 'success');
    } catch (err) {
      console.error(err);
  ...
  const editingDoc = () => ({
    ...
    order: editing.order,
    ...(editing.src && editing.src.startsWith('https://cdn.sanity.io')
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {editing && (
        ...
        >
          ...
          >
            <h3 style={{ margin: 0, color: '#FFD873' }}>{isNew ? 'Add Carousel Item' : 'Edit Carousel Item'}</h3>
            ...
            <div style={fieldStyle}>
              ...
              <input style={inputStyle} value={editing.alt} onChange={(e) => setEditing({ ...editing, alt: e.target.value })} />
            </div>

            <div style={fieldStyle}>
              <label style={labelStyle}>Description (shown in hover popup)</label>
              <textarea style={textareaStyle} rows={4} value={editing.description} onChange={(e) => setEditing({ ...editing, description: e.target.value })} />
            </div>
